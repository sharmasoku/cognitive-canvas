...
import { MessageSquare, Loader2, RefreshCw, Trash2, Mail, Phone, Calendar } from "lucide-react";
    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">{messages.length} customer messages</p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {messages.map((msg, i) => (
            <motion.div
              ...
              className="flex flex-col rounded-2xl border border-white/10 bg-[#111420] p-5 relative overflow-hidden"
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h3 className="font-semibold text-white text-sm">{msg.name}</h3>
                ...
                <button
                  ...
                  className="rounded-lg border border-red-500/20 p-2 text-red-400 hover:bg-red-500/10 transition"
              ...
              <div className="mt-4 flex-1 rounded-xl bg-[#0a0d14] p-3 text-sm text-gray-300 leading-relaxed whitespace-pre-wrap">
                {msg.message}
              </div>
            </motion.div>
          ))}
