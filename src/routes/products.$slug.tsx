      ...
      <div className="section-container py-8 sm:py-12 lg:py-16">
        ...
        <div className="grid gap-12 lg:grid-cols-2 lg:items-start lg:gap-20 xl:gap-24">
          ...
          <div>
            ...
            <div className="mt-9 grid grid-cols-2 gap-x-4 gap-y-5 border-t border-border-light pt-8 sm:grid-cols-4 sm:gap-x-3">
              <Feature icon={Truck} label="Free shipping" />
              <Feature icon={ShieldCheck} label="2-yr warranty" />
              <Feature icon={RefreshCw} label="14-day calibration" />
              <Feature icon={Zap} label="Lifetime OS updates" />
            </div>
          </div>
        </div>
        ...
        <tbody>
          {Object.entries(product.specifications).map(([k, v], i) => (
            <tr key={k} className={i % 2 === 0 ? "bg-surface" : "bg-background"}>
              <th className="w-1/3 px-4 py-3.5 text-left font-medium text-text-secondary sm:px-6">{k}</th>
...
function ReviewsList({ product, reviews, onSubmitted }: { product: Product; reviews: DbReview[]; onSubmitted: () => void }) {
  ...
  return (
    <div>
      <form onSubmit={handleSubmit} className="mb-6 rounded-2xl border border-border-light bg-surface p-5 sm:p-6">
        <div className="text-sm font-semibold">Write a review</div>
        <div className="mt-3 flex items-center gap-2">
          <span className="text-sm text-text-muted">Rating:</span>
          {[1, 2, 3, 4, 5].map((i) => (
            <button type="button" key={i} onClick={() => setRating(i)} className="p-0.5 transition hover:scale-110" aria-label={`${i} star`}>
      ...
      {sorted.length === 0 ? (
        ...
        <>
          ...
          <div className="space-y-3">
            {sorted.map((r) => (
              <div key={r.id} className="rounded-2xl border border-border-light bg-background p-5 sm:p-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2 font-semibold">
                        {r.userName}
                        {r.verified && <span className="rounded-full bg-surface-green px-2 py-0.5 text-[10px] font-medium text-accent-dark">Verified</span>}
                      </div>
